<template>
  <section class="gva-section [background:radial-gradient(720px_360px_at_50%_120%,rgba(34,100,242,0.12),transparent_70%),var(--gva-bg-base)]">
    <div class="gva-container text-center flex flex-col items-center">
      <h2 class="text-[clamp(28px,3.6vw,40px)] leading-[1.18] font-bold tracking-[-0.02em] text-[var(--gva-text-strong)] dark:text-[var(--gva-text-strong)] mt-4">强大的 API，<span class="text-[var(--gva-primary)]">更强大的 AI</span></h2>
      <p class="text-[clamp(15px,1.5vw,18px)] leading-[2] text-[var(--gva-text-body)] max-w-[820px] mt-4 mx-auto mb-8 max-[860px]:font-light">GVA 3.0，让 API 也能被 AI 理解、调用、自动化。</p>
      <div class="flex gap-3.5 flex-wrap justify-center">
        <ExperienceDropdown menu-align="left" class="max-[560px]:min-w-[140px] max-[560px]:flex-1" />
        <a class="inline-flex items-center justify-center gap-2 h-[43px] max-[860px]:h-12 px-7 rounded-[8px] text-[14px] font-normal cursor-pointer whitespace-nowrap border border-[var(--gva-primary)] transition-[transform,box-shadow,background-color,border-color,color] duration-200 bg-transparent text-[var(--gva-primary)] hover:border-[var(--gva-primary)] hover:text-[var(--gva-primary)] max-[560px]:flex-1 max-[560px]:min-w-[140px]" href="/guide/introduce/project">阅读文档 →</a>
      </div>
    </div>
  </section>
</template>

<script setup>
import ExperienceDropdown from '../ExperienceDropdown.vue'
</script>
