...
- 数据库：采用`MySql`>5.7版本,数据库引擎 innoDB<Badge type="danger" text="improtant" />，使用`gorm`实现对数据库的基本操作,已添加对sqlite数据库的支持。
...
## 主要功能
- 插件中心 <Badge type="tip" text="NEW" /> :基于 `Gva`自己的一套设计风格，独创 `go`的插件中心，现已支持 ：`微信支付、登录等`，`K8s相关操作` ，`第三方登录` 等等插件
